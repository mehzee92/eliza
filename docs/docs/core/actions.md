...
const ignoreAction: Action = {
  ...
  description: "Used when ignoring the user is appropriate (conversation ended, user is aggressive, etc.)",
...
const complexAction: Action = {
  ...
  handler: async (runtime, message, state) => {
    ...
    // Process document
    const content = await runtime.getService<IDocumentService>(
