...
export const getChainConfigs = (runtime: IAgentRuntime) => {
  return runtime.character.settings.chains?.evm as ChainConfig[] || DEFAULT_CHAIN_CONFIGS;
}
...
export class WalletProvider {
  ...
  async switchChain(runtime: IAgentRuntime, chain: SupportedChain): Promise<void> {
    const walletClient = this.chainConfigs[this.currentChain].walletClient
    if (!walletClient) throw new Error('Wallet not connected')

    try {
      await walletClient.switchChain({ id: getChainConfigs(runtime)[chain].chainId })
    } catch (error: any) {
      if (error.code === 4902) {
        ...
        await walletClient.addChain({
          chain: {
            ...
            rpcUrls: {
              default: { http: [getChainConfigs(runtime)[chain].rpcUrl] },
              public: { http: [getChainConfigs(runtime)[chain].rpcUrl] }
            }
          }
        })
        await walletClient.switchChain({ id: getChainConfigs(runtime)[chain].chainId })
